...
class HardwareManager:
    def __init__(self, port=None, baudrate=9600, mp3_path="RING.wav", twilio_sid=None, twilio_token=None, twilio_from=None):
    ...
    def _auto_detect_serial(self):
        ...
        for p in ports:
            if "Bluetooth" not in p.description: return p.device
    ...
    def _play_audio_async(self, file_path, repeat=1):
        if not self.pygame_imported: return
        def play():
            try:
    ...
    def alert_with_voice(self, active=True):
        ...
        if active:
            if os.path.exists(self.mp3_path): self._play_audio_async(self.mp3_path, repeat=2)
        else:
            if self.pygame_imported: self.pygame.mixer.music.stop()
    ...
    def call_emergency(self, to_number):
        if self.twilio_enabled:
            try:
                ...
                call = client.calls.create(url="http://demo.twilio.com/docs/voice.xml", to=to_number, from_=self.twilio_from)
    ...
    def close(self):
        if self.pygame_imported: self.pygame.mixer.quit()
